...
class Gutentor_Hooks {
	...
	function load_lib_assets() {
		...
		*/
		...
		if ( ! is_admin() ) {
			...
			*/
			if ( has_block( 'gutentor/counter-box' )
				|| has_block( 'gutentor/e3' ) ) {
			...
			*/
			if ( has_block( 'gutentor/progress-bar' )
				|| has_block( 'gutentor/e9' ) ) {
				/*Easy Pie Chart Js*/
	...
	function load_last_scripts() {
		...
		if ( ! $css_info || ! isset( $css_info['version'] ) || ! gutentor_get_options( 'gutentor_load_optimized_css' ) ) {
        ...
        if ( gutentor_is_edd_active() ) {
            wp_enqueue_style( 'gutentor-edd-css' );
            wp_style_add_data( 'gutentor-edd-css', 'rtl', 'replace' );
        }
